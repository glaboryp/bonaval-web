<template>
  ...
  >
    <div class="container">
      ...
      <div
        class="biblio"
        style="margin-top:1.75rem; font-size:.85rem; line-height:1.6;"
      >
        ...
        <span
          v-for="(b,i) in bibliografia"
          :key="b.url"
          style="display:inline-block; margin:.15rem .65rem .15rem 0;"
        >
          <a
            :href="b.url"
            target="_blank"
...
<script setup>
  defineProps({
    creditos: { type: Array, required: true },
    bibliografia: { type: Array, required: true },
    active: { type: Boolean, default: false }
  })
</script>

<style scoped>
.creditos { margin-bottom:10px; }
.biblio a { color: var(--accent2); text-decoration: none; }
.repo-container { margin-top: 2rem; }
.repo-link { display:inline-flex; align-items:center; gap:.5rem; background:#161a20; border:1px solid #242a33; color:#dfe6ef; border-radius:10px; padding:.6rem .8rem; text-decoration:none; }
.repo-link:hover { background:#1b2129; border-color:#2a333f; }
